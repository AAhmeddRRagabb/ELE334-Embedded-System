...
{    
  ...
  while(1)
    ...
    if(SW1&&SW2) {                     // both pressed
      GPIO_PORTF_DATA_R = 0x00;       // LED is blue 	 pf3: green   pf2: blue   pf1: red     010
    }
		...
		{                           
			if(SW1 && (!SW2)) {                // just SW1 pressed
				GPIO_PORTF_DATA_R = 0x19;     // LED is red
			} 
			
			else{                        
				if((!SW1)&&SW2){              // just SW2 pressed
					GPIO_PORTF_DATA_R = 0x13;   // LED is green
				}
				
				else{                        // neither switch
					GPIO_PORTF_DATA_R = 0x15;   // LED is off
				}
			}
    }
  }
